...
wir verwenden das momentan nur lokal. Aber Achtung: 
...
werden keine Änderungen mehr übernommen.
...
\begin{enumerate}
    ...
    \item Unser Datenmodell ist Abhängig von der JPA. Wir sind auf immer
...
Der zentrale Punkt des Ansatzes ist die Kapselung eines Stücks zusammenhängender
...
\textit{Usecases} sind, die wir mit unserer Entwicklung bedienen wollen. Bezüglich
der Granularität dieser Abbildung herrscht allerdings keine Einigkeit. Während 
...
Wir haben bereits eine ganz saubere Schichtentrennung und darüber hinaus
...
herkommt. Hier kommen die \textit{driven ports} oder \textit{out ports} ins
...
Man beachte das \textit{@Service} über der Klasse. Das wäre eigentlich von der
...
\textit{@Service}\ ändert nichts am Verhalten der Komponente und könnte bei der
...
Der Auftrag ist klar - wir müssen lediglich das Interface BookPersistence aus 
...
finalisieren. Es fehlt nur noch seine eigentliche Implementierung,
...
Nichts davon sollte wirklich überraschend sein ausser die beiden privaten
Funktionen \texttt{fromDomain} und \texttt{toDomain}. Da unsere
Entity-Klasse sehr einfach aufgebaut ist und lediglich Strings
benutzt, müssen wir nichts zwischen diesen beiden Repräsentationen umwandeln.
...
Service-übergreifenden I/O Funktionalität. Logging würde man identisch
...
(einen \textit{Adapter}) schreiben, der unsere \textit{driving ports}
...
Unittests benötigt hängt von der Komplexität der Adapter ab. Für unsere
...
Final testen wir nun noch unseren REST-Adapter (Listing \ref{lst:BookControllerTest}).
...
Validierung und ein Modell, das inkonsistente und falsche Repräsentationen
