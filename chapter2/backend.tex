...
(also nach Verlassen unseres Konstruktors) noch zu manipulieren. 
...
\end{minted}
\caption{Vollständig validierter OrderableItem Record}
...
\end{minted}
\caption{UnitPrice-Record mit Validation}
\label{lst:priceRecord}
\end{listing}

Listing \ref{lst:priceRecord} zeigt den UnitPrice-Record mit einer Validierung
...
und auch der \texttt{Componet} nur einmal vorkommen. Alleine daran sehen wir,
...
\end{minted}
\caption{MeasuredQuantity enum inkl. der Abbildung von Zahlenranges}
...
\end{minted}
\caption{Gekürzt dargestelltes SpecifiedProperty-enum}
...
wäre hingegen eine parametrische Suche (wie etwa ``gib mir alle Bauteile
...
\label{lst:SpecifiedPropertyEnumWithValidation}
\end{listing}

Die Früchte unserer Arbeit erkennen wir spätestens, wenn wir eine Komponente
...
Auch ohne, dass wir in die Implementierung schauen müssen, ist dieses Stück
...
Beim Testen haben wir nun gleich zwei Vorteile. Erstens werden wir weniger Tests
...
des Use Cases direkt \texttt{invoke} zu nennen aber davon ist abzusehen.
...
Durch dieses Vorgehen lässt sich jegliches I/O aus dem inneren unseres Hexagons
...
public class ComponentsUpdateService implements AddOrderableItemUseCase {
    ...
    private final StoragePersistenceAdapter storagePersistenceAdapter;
    ...
    public Component invokeUcAddItem(InternalPartNumber internalPartNumber,
        ...
        // *** Shell: I/O ***
        // *******************
        Component component = storagePersistenceAdapter.loadComponent(
        ...
        // *** Shell: I/O ***
        // *******************
        storagePersistenceAdapter.updateComponent(internalPartNumber, component);
...
entfernt werden. Wir fordern sogar, dass es keine nicht-zugeordneten
